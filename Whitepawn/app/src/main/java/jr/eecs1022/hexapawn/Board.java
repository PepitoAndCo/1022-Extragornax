...
public class Board
{
    ...
    public List<BoardPos> validWhiteMoves(BoardPos pos)
    {
        ...
        if (pos.getRow() == 0)  //如果胜利了
            return moves;
        if (getContents(pos.getRow() - 1, pos.getCol()) == Contents.EMPTY)  //如果正前方是空的
            moves.add(new BoardPos(pos.getRow() - 1, pos.getCol()));
        if ((pos.getCol() != 0) && (getContents(pos.getRow() - 1, pos.getCol() - 1) == Contents.BLACK_PIECE)) //如果没有到达左边界，且，左前方是空的
            moves.add(new BoardPos(pos.getRow() - 1, pos.getCol() - 1));
        if ((pos.getCol() != (WIDTH - 1)) && (getContents(pos.getRow() - 1, pos.getCol() + 1) == Contents.BLACK_PIECE)) //如果没有到达右边界，且，右前方是空的
            moves.add(new BoardPos(pos.getRow() - 1, pos.getCol() + 1));

        return moves;
    }

    public List<BoardPos> validBlackMoves(BoardPos pos)
    {
        if (getContents(pos) != Contents.BLACK_PIECE)
            throw new RuntimeException("There was no black piece at " + pos);
        List<BoardPos> moves = new ArrayList<BoardPos>();
        if (pos.getRow() == WIDTH - 1)  //如果胜利了
            return moves;
        if (getContents(pos.getRow() + 1, pos.getCol()) == Contents.EMPTY) //如果后方是空的
            moves.add(new BoardPos(pos.getRow() + 1, pos.getCol()));
        if ((pos.getCol() != 0) && (getContents(pos.getRow() + 1, pos.getCol() - 1) == Contents.WHITE_PIECE))
            moves.add(new BoardPos(pos.getRow() + 1, pos.getCol() - 1));
        if ((pos.getCol() != (WIDTH - 1)) && (getContents(pos.getRow() + 1, pos.getCol() + 1) == Contents.WHITE_PIECE))
            moves.add(new BoardPos(pos.getRow() + 1, pos.getCol() + 1));
    ...
    public void copyBoard(Board otherBoard){
        for (int r = 0; r < Board.WIDTH; r++) {
            for (int c = 0; c < Board.WIDTH; c++) {
                field[r][c] = otherBoard.getContents(r, c);
            }
        }
    }
}
